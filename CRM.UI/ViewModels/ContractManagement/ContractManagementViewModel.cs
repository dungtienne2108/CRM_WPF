using CommunityToolkit.Mvvm.ComponentModel;
using CommunityToolkit.Mvvm.Input;
using CRM.Application.Dtos.Contract;
using CRM.Application.Interfaces.Contract;
using CRM.Infrastructure.Services;
using CRM.UI.ViewModels.Base;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Win32;
using System.Collections.ObjectModel;
using System.Windows;

namespace CRM.UI.ViewModels.ContractManagement
{
    public partial class ContractManagementViewModel : ViewModelBase
    {
        private readonly IContractService _contractService;
        private readonly IServiceProvider _serviceProvider;

        [ObservableProperty]
        private string searchText = string.Empty;
        [ObservableProperty]
        private int currentPage;
        [ObservableProperty]
        private int recordsPerPage;
        [ObservableProperty]
        private int totalRecords;
        [ObservableProperty]
        private string totalRecordsText = string.Empty;
        [ObservableProperty]
        private bool isLoading;

        [ObservableProperty]
        private ObservableCollection<ContractItemViewModel> _contractItems = new();
        [ObservableProperty]
        private ContractItemViewModel? _selectedContractItem;

        public ContractManagementViewModel(IServiceProvider serviceProvider, IContractService contractService)
        {
            CurrentPage = 1;
            RecordsPerPage = 10;
            TotalRecords = 0;
            TotalRecordsText = "Tổng số bản ghi: 0";
            IsLoading = false;
            _serviceProvider = serviceProvider;
            _contractService = contractService;
        }

        public static readonly List<int> RecordsPerPageOptions = new() { 10, 25, 50, 100 };

        public int TotalPages => (int)Math.Ceiling((double)TotalRecords / RecordsPerPage);

        #region Commands
        [RelayCommand]
        private async Task SearchAsync()
        {
            CurrentPage = 1;
            await InitializeAsync();
        }

        [RelayCommand]
        private async Task CreateNewAsync()
        {
            var addContractViewModel = _serviceProvider.GetRequiredService<AddContractViewModel>();
            await addContractViewModel.LoadDataAsync();
            var addContractDialog = new Views.ContractManagement.AddContractDialog(addContractViewModel);
            addContractDialog.Owner = System.Windows.Application.Current.MainWindow;
            var res = addContractDialog.ShowDialog();

            await InitializeAsync();
        }

        [RelayCommand]
        private async Task Export()
        {
            var dialog = new SaveFileDialog
            {
                Title = "Xuất danh sách hợp đồng ",
                Filter = "Excel Workbook (*.xlsx)|*.xlsx",
                FileName = "ContractList.xlsx",
                InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop)
            };

            if (dialog.ShowDialog() == true)
            {
                var exportPath = dialog.FileName;
                var exportList = await _contractService.GetContractsAsync(new()
                {
                    PageNumber = 1,
                    PageSize = int.MaxValue
                });
                ExcelHelper.ExportToExcelFile(exportList.Items, exportPath, "Contracts");

                MessageBox.Show("Xuất file Excel thành công!", "Thành công",
            MessageBoxButton.OK, MessageBoxImage.Information);
            }
        }

        [RelayCommand]
        private async Task ChangeRecordsPerPage(int newRecordsPerPage)
        {
            RecordsPerPage = newRecordsPerPage;
            CurrentPage = 1;
            await InitializeAsync();
        }

        // Pagination
        [RelayCommand(CanExecute = nameof(CanExecuteFirstPage))]
        private async Task FirstPage()
        {
            CurrentPage = 1;
            await InitializeAsync();
        }
        private bool CanExecuteFirstPage()
        {
            return CurrentPage > 1;
        }

        [RelayCommand(CanExecute = nameof(CanExecutePreviousPage))]
        private async Task PreviousPage()
        {
            if (CurrentPage > 1)
            {
                CurrentPage--;
                await InitializeAsync();
            }
        }
        private bool CanExecutePreviousPage()
        {
            return CurrentPage > 1;
        }

        [RelayCommand(CanExecute = nameof(CanExecuteNextPage))]
        private async Task NextPage()
        {
            if (CurrentPage < TotalPages)
            {
                CurrentPage++;
                await InitializeAsync();
            }
        }
        private bool CanExecuteNextPage()
        {
            return CurrentPage < TotalPages;
        }

        [RelayCommand(CanExecute = nameof(CanExecuteLastPage))]
        private async Task LastPage()
        {
            CurrentPage = TotalPages;
            await InitializeAsync();
        }
        private bool CanExecuteLastPage()
        {
            return CurrentPage < TotalPages;
        }

        [RelayCommand]
        private async Task GoToPage(int pageNumber)
        {
            if (pageNumber >= 1 && pageNumber <= TotalPages)
            {
                CurrentPage = pageNumber;
                await InitializeAsync();
            }
            else
            {
                CurrentPage = 1;
                await InitializeAsync();
            }
        }

        [RelayCommand]
        private async Task OpenDetailAsync(ContractItemViewModel contractItem)
        {
            if (contractItem == null)
            {
                return;
            }

            var contractDetailViewModel = _serviceProvider.GetRequiredService<ContractDetailViewModel>();
            await contractDetailViewModel.LoadDataAsync(contractItem);
            var detailView = new Views.ContractManagement.ContractDetailView(contractDetailViewModel);
            detailView.Owner = System.Windows.Application.Current.MainWindow;
            detailView.ShowDialog();
        }
        #endregion

        #region Initialization
        public async Task InitializeAsync()
        {
            IsLoading = true;
            try
            {
                var getContractRequest = new GetContractRequest
                {
                    Keyword = SearchText,
                    PageNumber = CurrentPage,
                    PageSize = RecordsPerPage
                };

                var contractsResult = await _contractService.GetContractsAsync(getContractRequest);

                ContractItems.Clear();
                int index = (CurrentPage - 1) * RecordsPerPage + 1;

                foreach (var item in contractsResult.Items)
                {
                    ContractItems.Add(new(item, index++));
                }

                TotalRecords = contractsResult.TotalCount;
                TotalRecordsText = $"Tổng số: {TotalRecords} bản ghi";
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi khi tải dữ liệu hợp đồng: {ex.Message}", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
                IsLoading = false;
            }
        }
        #endregion
    }
}
